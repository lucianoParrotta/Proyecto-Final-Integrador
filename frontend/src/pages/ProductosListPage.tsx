...
const ProductosListPage: React.FC = () => {
  ...
  const proveedorNombreById = useMemo(() => {
    ...
    proveedores.forEach((p: any) => {
      map.set(Number(p.id), String(p.nombre ?? ""));
    });
    return map;
  }, [proveedores]);

  // Opciones de categorías derivadas de los productos reales
  const categorias = useMemo(() => {
    const set = new Set<string>();
    productos.forEach((p: any) => {
      const nombreCat = p?.categoria?.nombre ?? "";
      if (nombreCat) set.add(nombreCat);
    });
  ...
  const productosFiltrados = useMemo(() => {
    return productos.filter((p: any) => {
      ...
      const matchBusqueda = q.length === 0 || nombre.includes(q) || codigo.includes(q);
  ...
  const handleExportarXLS = () => {
    ...
    const rows = productosFiltrados.map((p: any) => {
      const categoriaNombre = p?.categoria?.nombre ?? "";
      const provNombre =
        p.proveedorId ? proveedorNombreById.get(Number(p.proveedorId)) ?? "" : "";
      const precioNumber = Number(p.precio);
      ...
      return [
        ...
        p.stock,
        p.stockMinimo,
        Number.isFinite(precioNumber) ? precioNumber : p.precio,
        p.estado,
        p.descripcion ?? "",
      ];
    });
    ...
    const rowsHtml = productosFiltrados
      .map((p: any) => {
        ...
        <tr>
          ...
          <td>${p.stock}</td>
          <td>${p.stockMinimo}</td>
          <td>$${(Number.isFinite(precioNumber) ? precioNumber : 0).toLocaleString("es-AR")}</td>
          <td>${p.estado}</td>
          <td>${p.descripcion ?? ""}</td>
        </tr>
      `;
      ...
      <html lang="es">
        <head>
          ...
          <style>
            ...
            caption { caption-side: bottom; margin-top: 8px; font-size: 11px; color: #6b7280; }
          </style>
        </head>
        <body>
          ...
          <table>
            <thead>
              <tr>
                <th>ID</th><th>Código</th><th>Nombre</th><th>Categoría</th><th>Proveedor</th>
                <th>Stock</th><th>Stock mínimo</th><th>Precio</th><th>Estado</th><th>Descripción</th>
              </tr>
            </thead>
  ...
  const handleConfirmExport = () => {
    if (!exportPDF && !exportXLS) {
      alert("Seleccioná al menos un formato.");
      return;
    }
  ...
  const handleEliminar = async (id: number) => {
    ...
    try {
      ...
      // Actualización optimista (rápida) + recarga suave opcional
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          ...
          <p className="text-slate-500 text-sm">Gestión de productos del inventario (datos reales).</p>
        </div>
      ...
      <section className="bg-white border border-slate-200 rounded-lg shadow-sm p-4 space-y-3">
        <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
          ...
          <div className="w-full md:w-56">
            <label className="block text-xs font-medium text-slate-500 mb-1">Estado / stock</label>
            <select
              className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-400"
              value={filtroEstado}
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                <th className="px-3 py-2 text-left text-xs font-semibold text-slate-500">Código</th>
                <th className="px-3 py-2 text-left text-xs font-semibold text-slate-500">Nombre</th>
                <th className="px-3 py-2 text-left text-xs font-semibold text-slate-500">Categoría</th>
                <th className="px-3 py-2 text-left text-xs font-semibold text-slate-500">Proveedor</th>
                <th className="px-3 py-2 text-right text-xs font-semibold text-slate-500">Stock</th>
                <th className="px-3 py-2 text-right text-xs font-semibold text-slate-500">Precio</th>
                <th className="px-3 py-2 text-center text-xs font-semibold text-slate-500">Estado</th>
                <th className="px-3 py-2 text-right text-xs font-semibold text-slate-500">Acciones</th>
              </tr>
            </thead>
              ...
              {productosFiltrados.map((p: any) => {
                ...
                return (
                  <tr key={p.id} className="border-b border-slate-100">
                    <td className="px-3 py-2 text-xs text-slate-700">{p.codigo}</td>
                    <td className="px-3 py-2 text-xs text-slate-700">{p.nombre}</td>
                    <td className="px-3 py-2 text-xs text-slate-600">{categoriaNombre}</td>
                    <td className="px-3 py-2 text-xs text-slate-600">{provNombre}</td>

                    <td className="px-3 py-2 text-xs text-right">
                      <span
                        className={
                          "inline-flex items-center justify-end gap-1 " +
                          (tieneStockBajo ? "text-amber-600" : "text-slate-700")
                        }
                      >
                        {p.stock}
                        {tieneStockBajo && (
                    ...
                    <td className="px-3 py-2 text-xs text-right text-slate-700">
                      ${Number.isFinite(precioNumber) ? precioNumber.toLocaleString("es-AR") : p.precio}
                    </td>
                    ...
                    <td className="px-3 py-2 text-xs text-right">
                      <div className="inline-flex gap-1">
                        ...
                        {/* ✅ Eliminar (agregado) */}
                        <button
                          onClick={() => handleEliminar(Number(p.id))}
                          disabled={isDeleting}
                          className="px-2 py-1 rounded-md border border-red-300 text-red-600 hover:bg-red-50 hover:border-red-400 transition disabled:opacity-60 disabled:cursor-not-allowed"
                        >
        ...
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md">
            <div className="px-6 pt-6 pb-4 border-b border-slate-100">
              ...
              <p className="text-xs text-slate-500 mt-1">Seleccioná uno o más formatos.</p>
            </div>
